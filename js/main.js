...
let wordsList = (str, subStr) => {
  ...
  .toLowerCase()
  .split (' ')
  .filter((wordsItem) => wordsItem.indexOf(subStr) > -1);
  let set = new Set();
...
Object.defineProperties (Car.prototype, {
  used: {
    get () {return this.year !== yearNow ? 'used' : 'new';},
    set (value) {if (value === 'new' && this.year < yearNow) this.year = yearNow;}
  }
})
...
function testPerformance(iterations, func) {
  ...
  let date = new Date();
  if (typeof func === 'function') for (let i = iterations; i < 1000; i++) func();
